...
//requestQueue for sequence request chat msgs
map<int, string> requestQueue;
...
int check_broadcast_list(int seq_no)
{
...
void ack_update(string ip, int seq_no)
{
    ...
    if(outer_tree != sent_message_details.end())
    {
        ...
        if(inner_tree != outer_tree->second.end())
        {
            ...
            int status = check_broadcast_list(seq_no);
            if(status == 1)
            {   
                cout << "erasing seq %d from senders queue"<< seq_no<<endl;
                sent_message_details.erase(seq_no);
            }
            else
            {
                cout << "waiting for ack for "<< seq_no<<endl;
            }
        }
        else
        {
...
void identify()
{
	switch(responseTag[0])
	{
		...
		else if(responseTag[0]=='S' && responseTag[1]=='0' && responseTag[2]=='_')
			{	
				//send ack to the requester indicating the request has been received
				generatedGlobalSeq =sequencer(createKey(clientAddress), atoi(responseLocalSeq));
